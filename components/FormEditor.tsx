...
export default function FormEditor({ existingForm }: FormEditorProps) {
  ...
  useEffect(() => {
    if (existingForm) {
      const formattedSections = existingForm.sections.map(section => ({
        name: section.name,
  ...
  const save = async () => {
    ...
    setLoading(true)
    try {
      const url = isEditing ? `/api/forms/${existingForm.id}` : '/api/forms'
      ...
      if (res.ok) {
        ...
        alert(`Failed to ${isEditing ? 'update' : 'save'} form`)
      }
    } catch {
      alert(`Failed to ${isEditing ? 'update' : 'save'} form`)
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex justify-between">
          ...
          <div className="flex gap-4">
            ...
            >
              {loading ? (isEditing ? 'Updating...' : 'Saving...') : (isEditing ? 'Update Form' : 'Save Form')}
            </button>
          </div>
        </div>
      </div>
